//This is a modified version of node-summary, by jbrooksuk
//Edited to allow multiple summary sentences, rather than just one
//Original code here: https://github.com/jbrooksuk/node-summary/
...
function getBestSentences(paragraph, sentences_dict, numSentences, callback) {
	splitContentToSentences(paragraph, function (sentences) {
		...
		var strip_s, sentence, s;
		for (s in sentences) {
			sentence = sentences[s]
			formatSentence(sentence, function (strip_s) {
				if (strip_s) {
					sentenceList.push({
						value: sentences_dict[strip_s],
						sentence: sentence
					});
				}
			})
		...
		var bestSentences = [];
		sentenceList.slice(0, numSentences).forEach((o) => {
			bestSentences.push(o.sentence);
		})
		callback(bestSentences);
	})
}
...
exports.summarize = function (title, content, numSentences, callback, sentences_dict) {
	...
	getSentencesRanks(content, function (dict) {
		splitContentToParagraphs(content, function (paragraphs) {
			...
			_.each(paragraphs, function (p) {
				getBestSentences(p, dict, numSentences, function (sentences) {
					summary = summary.concat(sentences);
				})
			})
